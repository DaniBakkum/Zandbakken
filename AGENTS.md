...
## Changelog
- 2026-06-01: Added an admin-only revision Excel export in the dashboard header. The export uses the already loaded row state, includes one row per sandpit with planning/location/revision fields, omits revision photos, and does not call storage APIs. Verified with `npm run lint` and `npm run build`.
- 2026-05-26: Hardened revision persistence after a production data-loss incident. Production now stores revisions separately under `zandbak-dashboard-revisions`, migrates existing inline row revisions on read/write, saves revision changes through a revision-specific `/api/rows` payload, and protects normal rows writes from overwriting existing revisions with stale browser snapshots. Rows-only saves now fail when the server reports `persisted: false`, and production Redis read failures no longer silently fall back to bundled rows when Redis is configured.
